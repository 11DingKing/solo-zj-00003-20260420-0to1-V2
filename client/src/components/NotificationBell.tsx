...
import { Notification, technicianApi, appointmentApi } from "@/lib/api";
import dayjs from "dayjs";
import toast from "react-hot-toast";

export default function NotificationBell() {
  const [isOpen, setIsOpen] = useState(false);
  const [userType, setUserType] = useState<"technician" | "customer">(
    "technician",
  );
  ...
  const handleConnect = async () => {
    ...
    try {
      if (userType === "technician") {
        const techId = parseInt(inputValue.trim(), 10);
        if (isNaN(techId)) {
          toast.error("请输入有效的技师ID");
          return;
        }
        await technicianApi.validate(techId);
        setUser({
          receiverId: inputValue.trim(),
          receiverType: userType,
        });
      } else {
        const response = await appointmentApi.validateCustomerPhone(
          inputValue.trim(),
        );
        if (response.data.exists) {
          ...
          toast.error("该手机号暂无预约记录");
        }
      }
    } catch (error: any) {
      if (userType === "technician") {
        if (error.response?.status === 404) {
          toast.error("技师不存在，请检查ID是否正确");
        } else {
            ...
            <div className="p-4 space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder={
                    userType === "technician" ? "请输入技师ID" : "请输入手机号"
                  }
            ...
            <>
              <div className="px-4 py-2 bg-gray-50 border-b border-gray-100 flex items-center justify-between">
                <span className="text-sm text-gray-600">
                  {user.receiverType === "technician" ? "技师" : "客户"}:{" "}
                  {user.receiverId}
                </span>
                <button
